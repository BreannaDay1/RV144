...
We now want to run a statisical test to see if the Aids vaccine was seemingly effective. We state the null and alternative hypothesis for this test.

$H_o:$ There's no relationship between getting Aids and having taken the Aids vaccine.

$H_a:$ There is a relationship between getting Aids and having taken the Aids vaccine.
...
We would expect to see both graphs of eq size (50 % on each side) if the two are independent. They do not appear to be equal ... sheddiing doubt on the idea of indepenence.
...
```{r}
myrv144 <-chisqtestGC(results, graph = TRUE) 
myrv144
```
  
  
  We now test the independence of aids vaccine and placebo groups.
  
$H_0:$ the placebo and aids vaccines treatment is independent of contracting the diesease.

$H_a:$ Having the vaccine is related to contracting the disease. 
  
 The resulting interpretation is the following. If the null hypothesis is indeed true the results we just obtained would happen about `r myrv144$p.value` percent of the time.

As a second approach we will consider the fisher exact test. We will state the null and alternate hypothesis for that test.

$H_0:$ The odds ratio between having aids and getting the vaccine contains the number 1.

$H_a:$ The odds ratio between having aids and getting the vaccine does not contain the number 1.

```{r}
myfisher <- fisher.test(results)
myfisher
```

The p.value from the fisher test is `r myfisher$p.value` and the confidence interval is `r myfisher$conf.int`.

## Conclusions

<Summarise your results as to the effectiveness of the vaccine with explanations that could be understood by your mom>
When we started to conduct the survey we see that those who took the placebo and aids vaccine had very low chance that they wouldn't get aids. Then we would conducted other test we seen that only 0.0482363 of the time the vaccine and placebo would work in combatting the virus. Thus, in this study we see that the placebo and vaccine didn't do exactly what we needed it to do.